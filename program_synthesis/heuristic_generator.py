...
class HeuristicGenerator(object):
    ...
    def prune_heuristics(self, heuristics, feat_combos, keep=1):
        ...
        acc_cov_scores = [f1_score(
            self.val_ground, L_val[:, i], average='micro') for i in range(np.shape(L_val)[1])]
    ...
    def run_synthesizer(self, max_cardinality=1, idx=None, keep=1, model='lr'):
        ...
        beta_opt = self.syn.find_optimal_beta(
            self.hf, self.val_primitive_matrix, self.feat_combos, self.val_ground)
    ...
    def run_verifier(self):
        ...
        self.vf = Verifier(self.L_train, self.L_val,
                           self.val_ground, has_snorkel=False)
    ...
    def heuristic_stats(self):
        ...
        # Make table
        column_headers = ['Feat 1', 'Feat 2', 'Val Acc',
                          'Train Acc', 'Val Cov', 'Train Cov']
